...
As illustrated in \cref{fig:management}d, the update mechanism operates per-head: Upon generating a new token $(k_t, v_t)$ with score $g_t$, the system (1) inspects the ``victim'' token at the current \textbf{local pointer}. If the victim's stored score satisfies $g \ge \tau$, it is (2) promoted to the global cache, advancing the \textbf{global pointer}. The new token then (3) overwrites the victim in the local cache, and the local pointer is incremented modulo $W_{\text{local}}$. To efficiently compute attention over the resulting ragged cache, we fold the head dimension into the batch dimension to leverage variable-length PagedAttention kernel, with details provided in Appendix~\ref{app:kernel}.